 ...
 *
 * Controls:
 * - GENIE_HOOK_PROFILE=minimal|standard|strict (default: standard)
 * - GENIE_DISABLED_HOOKS=comma,separated,hook,ids
 */
...
function getHookProfile() {
  const raw = String(process.env.GENIE_HOOK_PROFILE || 'standard').trim().toLowerCase();
  return VALID_PROFILES.has(raw) ? raw : 'standard';
}

function getDisabledHookIds() {
  const raw = String(process.env.GENIE_DISABLED_HOOKS || '');
