...
`ce-brainstorm` is the most common standalone entry point. Many teams skip `genie:think` (they already know what to explore) and skip `ce-plan` (the brainstorm is their full thinking artifact).
...
Skip `ce-brainstorm` and go directly to `/genie:plan`. The plan skill consumes any kind of input.
