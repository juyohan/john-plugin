...
For large bare-prompt scope (cross-cutting, sensitive surfaces, many files), `ce-work` recommends `/genie:brainstorm` or `/genie:plan` first — but proceeds with your choice.
...
- [`ce-plan`](./genie:plan.md) — produces the guardrails `ce-work` executes against
- [`ce-brainstorm`](./genie:brainstorm.md) — defines what the plan should accomplish
- [`genie:think`](./ce-ideate.md) — upstream "what's worth exploring" discovery
- [`ce-code-review`](./genie:review.md) — Tier 2 escalation target
- [`ce-commit-push-pr`](./genie:commit-push-pr.md) — handles the final commit + PR flow
- [`genie:learn`](./ce-compound.md) — capture reusable learning after shipping
