...
`/genie:think`는 `/genie:brainstorm`에 선행합니다.

- `/genie:think`는 "탐색할 가치가 있는 가장 강력한 아이디어는 무엇인가?"에 답합니다.
- `/genie:brainstorm`은 "선택된 하나의 아이디어가 정확히 무엇을 의미해야 하는가?"에 답합니다.
- `/genie:plan`은 "그것을 어떻게 구축해야 하는가?"에 답합니다.
...
3. **작업을 브레인스토밍으로 연결하십시오** - 아이디어 생성은 유망한 방향을 식별하며, `genie:brainstorm`은 선택된 방향을 계획 수립이 가능할 정도로 정밀하게 정의합니다. 아이디어 결과물에서 바로 계획 수립으로 넘어가지 마십시오.
...
- 해결 방향, 제약 조건, 대상 청중, 톤, 성공 기준 또는 주제의 특성을 묻지 마십시오 — 이는 `genie:brainstorm`의 영역입니다.
...
- 주제가 식별 가능해지거나 사용자가 "나를 놀라게 해줘"로 위임하면 즉시 중단하십시오. 0.2단계와 0.4단계를 통틀어 3개 이상의 질문을 하는 것은 아이디어 생성 워크플로우가 적절하지 않다는 신호입니다 — `genie:brainstorm`을 제안하는 것을 고려하십시오.
...
```bash
SCRATCH_DIR="/tmp/compound-engineering/think/<run-id>"
mkdir -p "$SCRATCH_DIR"
echo "$SCRATCH_DIR"
```

이후 이 실행에서의 모든 체크포인트 쓰기 및 캐시 읽기에 에코된 절대 경로(`/tmp/compound-engineering/think/<run-id>`)를 `<scratch-dir>`로 사용하십시오. 실행 디렉토리는 단계 6 완료 시 삭제되지 않습니다 — V15 캐시는 세션 범위이며 run-id 간에 재사용되고, 체크포인트는 나중의 호출에서 찾을 수 있도록 세션 범위 아티팩트를 남겨두는 컨벤션을 따릅니다.
