...
1.  **Brainstorm (`/ce-brainstorm`)**: 요구사항 확정.
2.  **Plan (`/ce-plan`)**: 파일·인터페이스 수준까지 구현 계획 확정. 언어 감지 후 언어별 스킬/룰 자동 제안.
3.  **TDD (`ecc:tdd-guide`)**: `ce-plan` 결과를 기반으로 실패하는 테스트 먼저 작성 (RED).
4.  **Work (`/ce-work`)**: 테스트를 통과하는 최소 구현 후 리팩토링 (GREEN → IMPROVE).
5.  **Review (ECC 기준)**: 언어별 reviewer (프로젝트 감지 자동 선택) + `security-reviewer`. CRITICAL 이슈는 머지 차단, HIGH는 머지 전 수정. ECC `rules/common/code-review.md` 체크리스트 적용.
...
| 역할 | 에이전트/명령어 | 출처 | 설명 |
| :--- | :--- | :--- | :--- |
| **Strategy** | `/ce-brainstorm` | CE | 요구사항 분석 및 전략 수립 |
| **Architect** | `architect` | ECC | 시스템 설계 및 아키텍처 결정 |
| **Planner** | `planner` | ECC | 파일·인터페이스 수준 구현 계획 |
| **TDD** | `tdd-guide` | ECC | 테스트 먼저 작성 (RED→GREEN→IMPROVE) |
| **Code Review** | `code-reviewer` | ECC | 일반 코드 품질·패턴·베스트 프랙티스 |
| **Language Review** | `typescript-reviewer`, `python-reviewer`, `go-reviewer`, `kotlin-reviewer`, `swift-reviewer`, `java-reviewer` | ECC | 언어별 전용 리뷰 (프로젝트 감지 후 자동 선택) |
| **Security** | `security-reviewer` | ECC | 보안 취약점·OWASP Top 10 감사 |
| **Build Fix** | `build-error-resolver`, `go-build-resolver`, `kotlin-build-resolver`, `swift-build-resolver`, `java-build-resolver` | ECC | 빌드·컴파일 에러 해결 |
| **Quality** | `refactor-cleaner`, `performance-optimizer`, `code-simplifier` | ECC | 리팩토링·성능·코드 단순화 |
| **E2E** | `e2e-runner` | ECC | 핵심 사용자 흐름 E2E 테스트 |
| **Docs** | `doc-updater` | ECC | 문서 업데이트 |
...
ideate·strategy 노트는 brainstorm 산출물과 같은 폴더에 suffix로 보관한다.
