...
| 브레인스토밍 전달 | `genie:brainstorm` | `genie:brainstorm` (universal-brainstorming) |
...
- **추천 다음 단계:** `/genie:brainstorm` (Proof 스킬의 최종 터미널 출력에 표시됨)
...
- **§6.3 선택한 아이디어 브레인스토밍.** Proof 반환이 성공하면(`proceeded` 또는 `done_for_now`), Phase 6 메뉴에서 멈추지 마십시오 — 아래의 상태별 처리를 적용한 후 §6.3의 나머지 단계(선택된 아이디어를 `Explored`로 표시, 그 후 `genie:brainstorm` 로드)를 계속하십시오. 오직 `aborted` 브랜치만 지속적인 기록이 작성되지 않았으므로 Phase 6 메뉴로 돌아갑니다.
...
3. **선택한 아이디어 브레인스토밍** — 선택한 아이디어를 시드로 하여 `genie:brainstorm`을 로드합니다. 오케스트레이터는 먼저 Phase 5의 모드 기본값을 사용하여 지속적인 기록을 작성합니다.
...
- 선택된 아이디어를 시드로 하여 `genie:brainstorm` 스킬을 로드하십시오.

**리포지토리 모드 전용:** 브레인스토밍을 건너뛰고 바로 `genie:plan`으로 가지 마십시오 — `genie:plan`은 브레인스토밍에 기반한 요구사항을 필요로 합니다. 기타 모드에서 아이디어 도출(또는 Proof 반복을 포함한 아이디어 도출)은 그 자체로 유효한 최종 상태입니다. 브레인스토밍은 하나의 아이디어를 더 깊게 발전시키기 위한 선택 사항이지, 존재하지 않는 구현 단계로 가기 위한 필수 단계가 아닙니다.
...
- 아이디어에 대한 조치는 구현으로 바로 가는 것이 아니라 `genie:brainstorm`으로 연결됨
