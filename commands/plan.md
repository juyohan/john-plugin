...
> 산출물을 출력한 뒤 대기합니다. 다음 단계 (`/genie:test`)는 사용자가 직접 실행합니다.
...
- **From a GitHub issue** — `/genie:plan https://github.com/.../issues/1234` (or paste the issue body)
- **From a PRD** — `/genie:plan` with the PRD path; the planning bootstrap reads it as origin
- **From a rough idea** — `/genie:plan "add background email digest at 8am UTC"` runs the bootstrap; the synthesis lets you correct scope before research dispatches
- **Re-deepening an existing plan** — `/genie:plan deepen the auth-rewrite plan` — interactive mode where agents present findings one by one for accept/reject
- **Cross-repo planning** — `/genie:plan "fix the busyblock bug in cli-printing-press"` from a different repo; the cross-repo target is announced and the plan lands in the target's `docs/plans/`
