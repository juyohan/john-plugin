...
- **권장 다음 단계 (Recommended next step)** (선택 사항, 호출자 전용): 호출자가 최종 터미널 출력에 표시되길 원하는 짧은 문자열입니다 (예: "Recommended next: `/genie:plan`"). 사용자가 직접 호출하는 경우에는 사용되지 않으며, 터미널 보고서는 단순히 반복 작업을 요약하고 다음 단계를 묻습니다.
