...
- `/tmp/compound-engineering/ce-code-review/<run-id>/`에 결과물 작성 (발견 사항, 수정 사항, 잔여 작업, 자문 출력).
...
`/tmp/compound-engineering/ce-code-review/<run-id>/`에 작성 (Interactive/autofix/headless). `metadata.json` 포함:
...
`fixes_applied_count > 0`일 때만 실행. 카운터 0이면 완료 보고서 후 종료.

- **PR 모드:** Push fixes / Exit
- **브랜치 모드 (PR 없음):** Create a PR (Recommended) / Continue without PR / Exit
- **베이스/기본 브랜치:** Continue / Exit

"Create a PR": `git push --set-upstream origin HEAD` 후 `gh pr create`. "Push fixes": `git push`.
