...
`.context/compound-engineering/optimize/<spec-name>/` 아래의 파일들은 로컬 임시 상태입니다. 이 파일들은 git에서 무시되므로 동일한 머신에서의 로컬 재개 시에는 유지되지만, 사용자가 별도로 내보내지 않는 한 커밋, 브랜치 또는 푸시를 통해 보존되지 않습니다.
...
### 파일 위치 (모두 `.context/compound-engineering/optimize/<spec-name>/` 아래)
...
6. 사양을 `.context/compound-engineering/optimize/<spec-name>/spec.yaml`에 기록합니다.
...
**브랜치가 존재하는 경우**, `.context/compound-engineering/optimize/<spec-name>/experiment-log.yaml`에 기존 실험 로그가 있는지 확인합니다.
...
작업 디렉토리를 생성합니다:
```bash
mkdir -p .context/compound-engineering/optimize/<spec-name>/
```
...
1. `.context/compound-engineering/optimize/<spec-name>/experiment-log.yaml`에 실험 로그 파일을 생성합니다.
...
4. **전략 요약 기록** — `.context/compound-engineering/optimize/<spec-name>/strategy-digest.md`에 다음 내용을 기록합니다:
...
1. **`/genie:review` 실행**: 누적된 diff(베이스라인에서 최종까지)에 대해 실행합니다. 최적화 브랜치에서 `mode:autofix`로 `genie:review` 스킬을 로드합니다.
2. **`/genie:learn` 실행**: 승리한 전략을 조직의 학습 내용으로 문서화합니다.
...
# 임시 배치 결과물 제거
rm -f .context/compound-engineering/optimize/<spec-name>/strategy-digest.md
```
